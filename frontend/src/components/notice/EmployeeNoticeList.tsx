  ...
  return (
    <div className="space-y-3">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-100 sm:hidden">{loading ? <p className="py-12 text-center text-sm text-slate-400">로딩 중...</p> : rows.length === 0 ? <p className="py-12 text-center text-sm text-slate-400">등록된 공지사항이 없습니다.</p> : rows.map((row) => <article key={row.noticeId} className={row.pinned ? "space-y-2 bg-amber-50/70 px-4 py-4" : "space-y-2 px-4 py-4"}><div className="flex items-start justify-between gap-3"><span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${row.pinned ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{noticeType(row)}</span><span className="text-xs text-slate-400">{formatDate(row.createdAt)}</span></div><button type="button" onClick={() => openView(row.noticeId)} className="block w-full text-left text-sm font-semibold text-slate-800 hover:text-indigo-600">{row.pinned && <span className="mr-1 text-amber-500">📌</span>}{row.title}{row.pinned && <span className="ml-2 rounded bg-rose-500 px-1 py-0.5 text-[10px] text-white">NEW</span>}</button><div className="flex items-center justify-between text-xs text-slate-500"><span>{row.writerName} · 조회 {row.viewCount}</span><button type="button" onClick={() => openView(row.noticeId)} className="rounded border border-slate-200 bg-slate-50 px-2 py-1 font-semibold text-slate-600">상세보기</button></div></article>)}</div>
